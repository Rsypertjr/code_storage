...
export default function TimeseriesChart({ state }: TimeseriesChartProps) {
  ...
  const loadTimeseriesData = useCallback(async () => {
    ...
    try {
      // Use the new timeseries API that handles both individual and combined files
    ...
    <div className="bg-white p-6 rounded-lg shadow-lg">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center text-sm">
        <div className="bg-blue-50 p-3 rounded">
          ...
          <div className="text-blue-900">{timeseriesData.data.length > 0 ? timeseriesData.data[timeseriesData.data.length - 1].total_votes.toLocaleString() : 'N/A'}</div>
        </div>
        <div className="bg-blue-50 p-3 rounded">
          <div className="font-semibold text-blue-700">Biden Final</div>
          <div className="text-blue-900">{timeseriesData.data.length > 0 ? timeseriesData.data[timeseriesData.data.length - 1].biden_votes?.toLocaleString() || 'N/A' : 'N/A'}</div>
        </div>
        <div className="bg-red-50 p-3 rounded">
          <div className="font-semibold text-red-700">Trump Final</div>
          <div className="text-red-900">{timeseriesData.data.length > 0 ? timeseriesData.data[timeseriesData.data.length - 1].trump_votes?.toLocaleString() || 'N/A' : 'N/A'}</div>
        </div>
        <div className="bg-gray-50 p-3 rounded">
          <div className="font-semibold text-gray-700">Records</div>
          <div className="text-gray-900">{timeseriesData.metadata.total_records || timeseriesData.data.length}</div>
        </div>
      </div>
    </div>
  )
